...
The cycle milestone's due date tracks the schedule: when it is opened the due date is the new-component/feature merge deadline (the Monday before the first Wednesday of its month — new components and big features should be merged by then to make the release; after it they generally wait for the next one, while bug fixes still go in), and when the first beta is cut the due date is moved to release day (the third Wednesday of the month).
