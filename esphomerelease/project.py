...
class Project:
    ...
    def cherry_pick_from_milestone(self, milestone: Milestone) -> List[Issue]:
        ...
        for issue in self.repo.issues(milestone=milestone.number, state='closed'):
            ...
            if not pull.is_merged():
                log = click.style(f"Not merged yet: {pull.title}\nIf you want to add it please merge "
                                  f"it manually then confirm.", fg='yellow')
    ...
    def create_pr(self, *, title: str, target_branch: BranchType,
        ...
        gprint(f"Created Pull Request #{pr.number} from {self.branch} against {target_branch}")
    ...
    def has_local_changes(self) -> bool:
        try:
            self.run_git('diff-index', '--quiet', 'HEAD', '--', fail_ok=True, silent=True)
            return False
...
ALL_PROJECTS: List[Project] = [
    EsphomeProject, EsphomeDocsProject, EsphomeHassioProject, EsphomeIssuesProject,
